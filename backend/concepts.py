...
def getConcepts() -> None:
    ...
    for idx, v in stock_board_concept_name_em_df.iterrows():
        ...
        print(f"[getConcepts] 当前concepts记录数: {len(concepts)}，concept_stocks记录数: {len(concept_stocks)}")
        if len(concepts) % 20 == 0:
            print("[getConcepts] 暂停15秒避免请求过于频繁")
            t.sleep(15)

    print(f"[getConcepts] 数据获取完成，concepts总记录数: {len(concepts)}，concept_stocks总记录数: {len(concept_stocks)}")
    ...
    print(f"[getConcepts] 概念数据已保存到: {concepts_csv_path}")
    df_concept_stocks.to_csv(concept_stocks_csv_path, index=False)
    print(f"[getConcepts] 概念成分股数据已保存到: {concept_stocks_csv_path}")
