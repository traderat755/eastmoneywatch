...
def prepareChanges():
    ...
    for code, type_value in type_mapping.items():
        ...
        print(f'[prepare] {type_value} 返回数据shape: {df.shape}')
        df['类型'] = type_value
        all_df.append(df)
        print('[prepare] 等待5秒...')
        time.sleep(5)
    df = pd.concat(all_df, ignore_index=True)
    print(f'[prepare] 拼接后总数据shape: {df.shape}')
    ...
    if os.path.exists(concepts_path):
        ...
        # 为df添加板块信息
        df['板块'] = df['名称'].map(stock_to_sector_map)

        # 处理未找到板块的股票
        missing_sector_count = df['板块'].isna().sum()
        if missing_sector_count > 0:
            print(f'[prepare] 有{missing_sector_count}只股票未找到对应板块，使用默认板块')
            df['板块'] = df['板块'].fillna('未知板块')
